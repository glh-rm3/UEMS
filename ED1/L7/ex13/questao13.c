...
13. Uma ABB e estritamente binária se todos os nós da árvore tem 2 filhos. Implemente uma função que
...
int strictly_binary(struct NO* atual) {
    if(atual == NULL) 
        return 1;
    ...
    return strictly_binary(atual->esq) && strictly_binary(atual->dir);
}

int is_sbinary(ArvBin *raiz) {
    if(raiz == NULL) return -1;
    
    return strictly_binary(*raiz);
}
...
int main() {
    ...
    */
    ...
    imprime(raiz);
    if(is_sbinary(raiz)) {
        puts("Eh estritamente binaria!");
    } else 
        puts("Nao eh estritamente binaria!");
